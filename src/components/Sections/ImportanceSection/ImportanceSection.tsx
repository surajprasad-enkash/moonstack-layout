...
const ImportanceSection: React.FC<IImportanceSectionProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`${bgColor} relative container mx-auto flex flex-col items-center px-4 py-12 md:flex-row md:px-8 md:py-20 ${
          ...
          >
            {description.map((desc, index) => (
              <Heading
                key={index}
                headingTag="p"
                className="pt-3 text-start font-medium"
            ...
            <ul className="flex flex-col gap-3 pt-8">
              {points.map((text, index) => (
                <li key={index} className="flex items-start gap-4 pb-3">
                  ...
                  <Heading
                    headingTag="p"
                    className="text-start font-medium text-white"
                    content={[{ text }]}
                  />
                </li>
              ))}
