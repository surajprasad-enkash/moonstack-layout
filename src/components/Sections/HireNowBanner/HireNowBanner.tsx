...
const HireNowBanner: React.FC<HireNowBannerProps> = ({
  ...
  return (
    <section className="relative bg-black">
      ...
      >
        <div
          className={`relative overflow-hidden rounded-[30px] px-6 py-10 md:rounded-[40px] md:px-[60px] md:py-20 ${!bgImage ? bgGradient : ""}`}
          style={{
        ...
        >
          ...
          >
            ...
            {/* Image Section */}
            <div
              className={`flex justify-center md:justify-end ${reverse ? "md:order-1" : "md:order-2"}`}
            >
              <div
                className={`absolute bottom-0 left-1/2 hidden h-[220px] w-full -translate-x-1/2 md:left-auto md:block md:h-[327px] md:translate-x-0 ${imageHeight}`}
              >
                <Image
                  src={image}
                  alt="banner-image"
